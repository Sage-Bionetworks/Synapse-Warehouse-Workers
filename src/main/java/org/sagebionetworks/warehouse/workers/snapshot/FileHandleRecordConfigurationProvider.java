...
public class FileHandleRecordConfigurationProvider implements WorkerStackConfigurationProvider {

	final WorkerStackConfiguration config;

	@Inject
	public FileHandleRecordConfigurationProvider (CountingSemaphore semaphore,
			AmazonSQSClient awsSQSClient, AmazonSNSClient awsSNClient,
			FileHandleRecordWorker worker, FileHandleRecordTopicBucketInfo config,
		...
		mdwsc.setSemaphoreLockKey(SemaphoreKey.FILE_HANDLE_RECORD_WORKER.name());
		...
		this.config.setWorkerName(FileHandleRecordWorker.class.getName());
	}
